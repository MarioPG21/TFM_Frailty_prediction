#!/usr/bin/env python3
"""
Publica meses de la Fuente A (registros clínicos CSV) al bucket landing de MinIO.

Uso (desde la raíz del proyecto):
    python spark-apps/scripts/publish_clinical.py --ticks 1
    python spark-apps/scripts/publish_clinical.py --ticks all --delay 5
    python spark-apps/scripts/publish_clinical.py --ticks all --delay 0
"""
import argparse
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Publica Fuente A a MinIO landing")
    ...
    source_dir = _SYNTHETIC / "source_a" / "clinical_records"
    ...
    if args.day:
        ...
        print(f"[{stem}] clinical → s3://{bucket}/{key}  ({n_rows:,} registros)")
        return

    pending = []
    for fpath in all_files:
        year, month = fpath.stem.split("-")
        key = f"clinical/{year}/{month}/{fpath.name}"
    ...
    for i, (fpath, key) in enumerate(to_publish):
        n_rows = sum(1 for _ in open(fpath, encoding="utf-8")) - 1
        client.upload_file(str(fpath), bucket, key)
        print(f"[{fpath.stem}] clinical → s3://{bucket}/{key}  ({n_rows:,} registros)")
