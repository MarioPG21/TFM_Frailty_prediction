#!/usr/bin/env python3
"""
Publica oleadas de la Fuente B2 (encuestas de hábitos, JSONL) al bucket landing de MinIO.

Uso:
    python spark-apps/scripts/publish_lifestyle.py --ticks 1
    python spark-apps/scripts/publish_lifestyle.py --ticks all --delay 0
"""
import argparse
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Publica Fuente B2 a MinIO landing")
    ...
    source_dir = _SYNTHETIC / "source_b2" / "lifestyle_surveys"
    ...
    if args.day:
        ...
        if not matches:
            print(f"[{stem}] lifestyle → sin datos para este mes")
            return
        fpath = matches[0]
        key = f"lifestyle/{d.year:04d}/{d.month:02d}/{fpath.name}"
        if _exists(client, bucket, key):
            print(f"[{stem}] lifestyle → ya publicado, omitiendo")
            return
        n_rows = sum(1 for _ in open(fpath, encoding="utf-8"))
        client.upload_file(str(fpath), bucket, key)
        print(f"[{stem}] lifestyle → s3://{bucket}/{key}  ({n_rows:,} registros)")
        return

    pending = []
    for fpath in all_files:
        year, month = fpath.stem.split("-")
        key = f"lifestyle/{year}/{month}/{fpath.name}"
    ...
    for i, (fpath, key) in enumerate(to_publish):
        n_rows = sum(1 for _ in open(fpath, encoding="utf-8"))
        client.upload_file(str(fpath), bucket, key)
        print(f"[{fpath.stem}] lifestyle → s3://{bucket}/{key}  ({n_rows:,} registros)")
