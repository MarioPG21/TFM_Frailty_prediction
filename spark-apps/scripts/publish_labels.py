...
def main():
    ...
    if args.day:
        ...
        if not matches:
            print(f"[{stem}] labels → sin datos para este mes")
            return
        fpath = matches[0]
        key = f"labels/{d.year:04d}/{d.month:02d}/{fpath.name}"
        if _exists(client, bucket, key):
            print(f"[{stem}] labels → ya publicado, omitiendo")
            return
        n_rows = sum(1 for _ in open(fpath, encoding="utf-8")) - 1
        client.upload_file(str(fpath), bucket, key)
        print(f"[{stem}] labels → s3://{bucket}/{key}  ({n_rows:,} etiquetas)")
        return
