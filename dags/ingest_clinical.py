"""
DAG de ingesta clínica (Fuente A).

Cada tick lee los próximos INGEST_WINDOW_MINUTES de datos simulados desde
la landing zone y los procesa en Bronze + Silver. Si la ventana no contiene
registros nuevos, el DAG termina en estado Skipped (sin consumir cores Gold).
...
def _run(command: str) -> None:
    """Ejecuta spark-submit <command>. Lanza AirflowSkipException si no hay datos nuevos."""
...
def ingest_clinical_dag():

    @task
    def run_bronze():
        _run("bronze_clinical")

    run_silver = BashOperator(
        task_id="run_silver",
        bash_command=_SUBMIT + " silver_clinical",
